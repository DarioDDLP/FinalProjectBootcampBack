...
const getByPostId = (postId) => {
    return executeQuery(`SELECT tm.id, tm.created_at as 'created_at', t.title as 'titlePost', tm.data as 'text', CONCAT(u.name, ' ', u.surname) as 'username' FROM thread_messages as tm JOIN threads as t on t.id = tm.thread_id  join users as u on tm.users_id = u.id where tm.thread_id = ? and tm.status = 1`, [postId]);
}
